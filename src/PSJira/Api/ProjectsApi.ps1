...
function Update-ProjectType {
    ...
    Process {
        'Calling method: Update-ProjectType' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug
        ...
        $LocalVarUri = '/rest/api/3/project/{projectIdOrKey}/type/{newProjectTypeKey}'
        if (!$ProjectIdOrKey) {
            throw "Error! The required parameter `ProjectIdOrKey` missing when calling updateProjectType."
        }
        $LocalVarUri = $LocalVarUri.replace('{projectIdOrKey}', [System.Web.HTTPUtility]::UrlEncode($ProjectIdOrKey))
        if (!$NewProjectTypeKey) {
            throw "Error! The required parameter `NewProjectTypeKey` missing when calling updateProjectType."
        }
        $LocalVarUri = $LocalVarUri.replace('{newProjectTypeKey}', [System.Web.HTTPUtility]::UrlEncode($NewProjectTypeKey))
